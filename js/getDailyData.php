...
if ($q == 1) {
	...
	$startDate = $startDate->createFromFormat('Y-m-d', '2016-11-01');
}
...
while ($record = mysqli_fetch_array($myData)) {
	if ($gameDate != $record['GAME_DATE']) {
		if ($gameDate != "") {
			echo "</table></div>";
			if ($month != date("m", strtotime($record['GAME_DATE']))) {
				echo "</section>";
			}
		...
		echo "<section id='Group" . $month . "' >";
		echo "<div id='Group" . $record['GAME_DATE'] . "' >";
	...
	if ($record['AWAY_SCORE'] != 0 && $record['HOME_SCORE'] != 0) {
		...
		if ($record['HOME_SCORE'] + $record['HOME_TEAM_SPRD'] - $record['AWAY_SCORE'] > 0) {
			echo "<td class='col-md-3' title='最近戰績： " . $record['AWAY_TEAM_TOTAL_STREAK'] .
				"\n例行賽戰績： " . $record['AWAY_SEASON_RECORD'] . "'>" . $record['AWAY_TEAM_NAME'] .
				" (" . $record['AWAY_TEAM_SPRD'] . ")" . "</td>";
			echo "<td class='col-md-3 bg-success'title='最近戰績： " . $record['HOME_TEAM_TOTAL_STREAK'] .
				"\n例行賽戰績： " . $record['HOME_SEASON_RECORD'] . "'>" . $record['HOME_TEAM_NAME'] .
				" (" . $record['HOME_TEAM_SPRD'] . ")" .
				"<small class='pull-right text-success bg-warning'>pass</small></td>";
		}
		//客場過盤
		else {
			echo "<td class='col-md-3 bg-success'title='最近戰績： " . $record['AWAY_TEAM_TOTAL_STREAK'] .
				"\n例行賽戰績： " . $record['AWAY_SEASON_RECORD'] . "'>" . $record['AWAY_TEAM_NAME'] .
				" (" . $record['AWAY_TEAM_SPRD'] . ")" .
				"<small class='pull-right text-success bg-warning'>pass</small></td>";
			echo "<td class='col-md-3'title='最近戰績： " . $record['HOME_TEAM_TOTAL_STREAK'] .
				"\n例行賽戰績： " . $record['HOME_SEASON_RECORD'] . "'>" . $record['HOME_TEAM_NAME'] .
				" (" . $record['HOME_TEAM_SPRD'] . ")" . "</td>";
		}
		//判斷賽果 客場勝
	...
	if ($record['AWAY_SCORE'] + $record['HOME_SCORE'] > $record['OVER_UNDER']) {
		echo "<td class='col-md-1 bg-danger'>" . $record['OVER_UNDER'] .
			" (" . $scoreTotal . ")" . "</td>";
	} else {
		echo "<td class='col-md-1'>" . $record['OVER_UNDER'] .
			" (" . $scoreTotal . ")" . "</td>";
	}
	echo "</tr>";
}
